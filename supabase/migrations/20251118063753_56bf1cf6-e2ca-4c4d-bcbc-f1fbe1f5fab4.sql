...
ADD COLUMN IF NOT EXISTS category public.problem_category DEFAULT 'other',
ADD COLUMN IF NOT EXISTS severity public.problem_severity DEFAULT 'medium',
ADD COLUMN IF NOT EXISTS language public.programming_language,
ADD COLUMN IF NOT EXISTS framework public.framework_type,
ADD COLUMN IF NOT EXISTS archived BOOLEAN DEFAULT false,
ADD COLUMN IF NOT EXISTS resolved_at TIMESTAMP WITH TIME ZONE,
ADD COLUMN IF NOT EXISTS ai_cost NUMERIC(10,2) DEFAULT 0.00,
...
CREATE TABLE IF NOT EXISTS public.problem_shares (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
DROP POLICY IF EXISTS "Users can view comments on their problems' insights" ON public.insight_comments;
...
USING (
    EXISTS (
        ...
        AND (p.user_id = auth.uid() OR EXISTS (
            ...
            WHERE ps.problem_id = p.id AND ps.shared_with_email = (SELECT email FROM auth.users WHERE id = auth.uid()) AND ps.accepted = true
        ))
    )
);

DROP POLICY IF EXISTS "Users can create comments on accessible insights" ON public.insight_comments;
...
DROP POLICY IF EXISTS "Users can delete their own comments" ON public.insight_comments;
...
DROP POLICY IF EXISTS "Users can view shares for their problems" ON public.problem_shares;
...
DROP POLICY IF EXISTS "Problem owners can create shares" ON public.problem_shares;
...
DROP POLICY IF EXISTS "Problem owners can update shares" ON public.problem_shares;
...
DROP POLICY IF EXISTS "Problem owners can delete shares" ON public.problem_shares;
...
DROP TRIGGER IF EXISTS log_problem_activity ON public.problems;
CREATE TRIGGER log_problem_activity
AFTER INSERT OR UPDATE ON public.problems
FOR EACH ROW
EXECUTE FUNCTION public.log_activity();

DROP TRIGGER IF EXISTS log_comment_activity ON public.insight_comments;
